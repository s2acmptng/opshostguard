        ...
        try {
            if ($isLocal) {
                ...
                Invoke-Command -ComputerName $targetHost -Credential $PsRemotingCredential -ScriptBlock {
                    ...
                    if (-not (Get-PackageProvider -Name NuGet -ErrorAction SilentlyContinue)) {
                        Install-PackageProvider -Name NuGet -Force -Scope LocalMachine
                    }
                    if (-not (Get-Module -ListAvailable -Name PSWindowsUpdate)) {
                        Install-Module -Name PSWindowsUpdate -Force -Scope LocalMachine
                    }
                    Import-Module PSWindowsUpdate
                    Update-WUModule -Online -Verbose
                }
                ...
                Invoke-WUJob -RunNow -ComputerName $targetHost -Credential $PsRemotingCredential -Force -Script {
                    $ConfirmPreference = 'None'
                    $ErrorActionPreference = 'SilentlyContinue'
                    # Ejecutar Get-WindowsUpdate y validar la salida
                    $updates = Get-WindowsUpdate -Verbose -MicrosoftUpdate -AcceptAll -AutoReboot -ErrorAction SilentlyContinue
                    ...
                    if ($null -ne $updates -and $updates.Count -gt 0) {
                        ...
                        $updates | Where-Object { $_.Title -notlike "Preview" } | ForEach-Object {
                            try {
                                ...
                                $installResult = Install-WindowsUpdate -Verbose -AcceptAll -IgnoreReboot -ErrorAction SilentlyContinue
                                [PSCustomObject]@{
                                    HostName  = $env:COMPUTERNAME
                                    Update    = $_.Title
                        ...
                        Write-Host "No updates available or an error occurred for $env:COMPUTERNAME."
                        [PSCustomObject]@{
                            HostName  = $env:COMPUTERNAME
                            Update    = "No updates"
                            Status    = "Skipped"
